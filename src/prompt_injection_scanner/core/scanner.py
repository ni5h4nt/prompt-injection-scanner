...
def log_stage(stage_name: str):
    ...
    def decorator(stage_func):
        ...
        async def wrapper(self, context: ScanContext) -> ScanContext:
            logger.debug(
                "stage_started", stage=stage_name, request_id=context.request_id
            )
...
def cache_result(ttl: int = 300):
    """Decorator for caching stage results - abstracts cache complexity"""

    def decorator(stage_func):
        @wraps(stage_func)
        async def wrapper(self, context: ScanContext) -> ScanContext:
            # Simple in-memory cache for now - could be extended to Redis
            cache_key = f"stage_{stage_func.__name__}_{hash(context.prompt)}"

            # For now, just execute without caching (placeholder implementation)
            # In a full implementation, this would check cache first
            return await stage_func(self, context)
...
def error_handler(stage_func):
    """Decorator for stage error handling - abstracts exception management"""
    ...
    async def wrapper(self, context: ScanContext) -> ScanContext:
        try:
            ...
            # Continue pipeline with current context instead of failing
            return context
...
class Scanner:
    ...
    def __init__(
        self, heuristic_stage: Stage, vector_stage: Stage, guardian_stage: Stage
    ):
        self.stages = [
    ...
    async def scan(
        self, prompt: str, request_id: str = None, metadata: Dict[str, Any] = None
    ) -> ScanResult:
