...
class PatternManager:
        ...
        try:
            if pattern.pattern_type == PatternType.REGEX:
                ...
                keywords = (
                    ...
                    if pattern.case_sensitive
                    else pattern.pattern.lower().split()
                )
            ...
            logger.error("pattern_match_error", pattern_id=pattern.id, error=str(e))
    ...
    def get_pattern_stats(self) -> PatternStats:
        ...
        if self._stats_cache is None:
            ...
            for pattern in self.patterns.values():
                ...
                severities[pattern.severity.value] = (
                    severities.get(pattern.severity.value, 0) + 1
                )
            ...
            self._stats_cache = PatternStats(
                ...
                enabled_patterns=len(
                    self.patterns
                ),  # Only enabled patterns are indexed
    ...
    def delete_pattern(self, pattern_id: str) -> bool:
        ...
        for pattern_set in self.pattern_sets.values():
            pattern_set.patterns = [
                p for p in pattern_set.patterns if p.id != pattern_id
            ]
    ...
    def bulk_add_patterns(
        self, patterns: List[Pattern], pattern_set_name: str = "bulk"
    ) -> Dict[str, Any]:
    ...
    def validate_pattern(self, pattern: Pattern) -> List[str]:
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", pattern.id):
            errors.append(
                "Pattern ID can only contain letters, numbers, underscores, and hyphens"
            )
