...
class MemoryVectorDB(VectorDB):
    ...
    async def search(
        ...
        """Search for similar embeddings using cosine similarity (legacy method)"""
    ...
    async def similarity_search(
        ...
        for similarity, doc in top_results:
            results.append(
                SearchResult(
                    similarity=similarity, text=doc["text"], metadata=doc["metadata"]
                )
            )

        logger.debug(
            "memory_db_similarity_search",
