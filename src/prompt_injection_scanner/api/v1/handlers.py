...
from .models import (
    ScanRequest, ScanResponse, ErrorResponse, HealthResponse, 
    BatchScanRequest, BatchScanResponse, StageResult
)
...
class ScanHandlerV1:
    ...
    async def scan_prompt(self, request: ScanRequest) -> ScanResponse:
        ...
        for stage_name, score in result.stage_scores.items():
            stage_result = StageResult(
                ...
                flags=[flag for flag in result.flags if stage_name in flag.lower()] or [],
    ...
    async def batch_scan(self, request: BatchScanRequest) -> BatchScanResponse:
        ...
        logger.info("v1_batch_scan_started", batch_id=batch_id, prompt_count=len(request.prompts))
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error("batch_scan_item_failed", batch_id=batch_id, item_index=i, error=str(result))
                # Create error result
        ...
        logger.info("v1_batch_scan_completed", 
                   batch_id=batch_id, 
                   total_processed=len(successful_results),
                   processing_time_ms=processing_time)
