...
from ...patterns import PatternManager, Pattern, PatternSet, PatternStats
from ...patterns.models import Severity, PatternType
...
async def list_pattern_sets(
    ...
    try:
        return {
            "pattern_sets": [ps.model_dump() for ps in pattern_manager.pattern_sets.values()],
...
async def get_pattern_set(
    ...
    try:
        if set_name not in pattern_manager.pattern_sets:
            raise HTTPException(status_code=404, detail=f"Pattern set '{set_name}' not found")
...
async def remove_pattern_set(
    ...
    try:
        if pattern_manager.remove_pattern_set(set_name):
            ...
            return PatternResponse(
                success=True,
                message=f"Pattern set '{set_name}' removed successfully"
            )
        else:
            raise HTTPException(status_code=404, detail=f"Pattern set '{set_name}' not found")
...
async def add_pattern(
    ...
    try:
        ...
        logger.info("pattern_added_via_api", pattern_id=pattern.id, set_name=pattern_set_name)
...
async def bulk_add_patterns(
    ...
    try:
        ...
        logger.info("bulk_add_patterns_via_api", 
...
async def bulk_update_patterns(
    ...
    try:
        ...
        logger.info("bulk_update_patterns_via_api",
...
async def bulk_delete_patterns(
    ...
    try:
        ...
        logger.info("bulk_delete_patterns_via_api",
...
async def enable_pattern(
    ...
    try:
        if pattern_manager.enable_pattern(pattern_id):
            return PatternResponse(
                success=True,
                message=f"Pattern '{pattern_id}' enabled successfully"
            )
        else:
            raise HTTPException(status_code=404, detail=f"Pattern '{pattern_id}' not found")
...
async def upload_pattern_file(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid pattern set format: {str(e)}")
        ...
        logger.info("pattern_file_uploaded",
                   filename=file.filename,
                   pattern_set=final_name,
                   patterns=len(pattern_set.patterns))
    ...
    try:
        ...
        for file in files:
            try:
                ...
                results["summary"]["total_patterns"] += upload_result.get("total_patterns", 0)
        ...
        logger.info("bulk_pattern_upload_completed",
                   total=len(files),
