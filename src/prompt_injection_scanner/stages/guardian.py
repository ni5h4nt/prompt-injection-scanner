...
from ..core.scanner import ScanContext, log_stage
...
def agent_runner(model_name: str):
    ...
    def decorator(func):
        # Lazy initialization - agent created when first needed
        agent = None
        ...
        async def wrapper(self, context: ScanContext) -> ScanContext:
            ...
            # Run agent with dependency injection (lazy init)
            agent = get_agent()
...
def confidence_booster(boost_factor: float):
    ...
    def decorator(func):
        async def wrapper(
            self, context: ScanContext, analysis: SecurityAnalysis
        ) -> ScanContext:
...
class GuardianStage:
    ...
    async def process(
        self, context: ScanContext, analysis: SecurityAnalysis, final_confidence: float
    ) -> ScanContext:
