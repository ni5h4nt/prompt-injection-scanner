...
def risk_calculator_dynamic(func):
    ...
    async def wrapper(self, context: ScanContext, matches: List[Dict[str, Any]]) -> ScanContext:
...
class HeuristicStage:
    ...
    def __init__(self, pattern_manager: PatternManager = None):
        ...
        logger.info("heuristic_stage_initialized", 
                   patterns=len(self.pattern_manager.patterns))
    
    @log_stage("heuristic")
    @early_exit(threshold=70)  # Exit early if heuristic finds high risk
    @risk_calculator_dynamic
    async def process(self, context: ScanContext, matches: List[Dict[str, Any]], risk_points: int) -> ScanContext:
        """
        ...
        high_severity_matches = [m for m in matches if m.get("severity") in ["high", "critical"]]
